<template>
  <header class="sticky top-0 z-50 bg-white/90 dark:bg-background-dark/90 backdrop-blur-lg border-b border-slate-100 dark:border-slate-800">
    <div class="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
      <!-- Logo -->
      <router-link :to="isLoggedIn && isProfessional ? '/dashboard' : '/'" 
      ...
      <nav class="hidden md:flex items-center gap-8">
        <template v-if="isLoggedIn">
          <template v-if="isProfessional">
            <router-link to="/dashboard" class="text-sm font-bold text-slate-600 hover:text-primary transition-all">
              Service Management
            </router-link>
          </template>
          <template v-else>
      ...
      <div class="flex items-center gap-2">
        ...
        <template v-if="!isLoggedIn">
          ...
          <router-link to="/signup"
            class="bg-primary text-white px-5 py-2.5 rounded-full font-bold text-sm shadow-md shadow-primary/30 hover:bg-primary-dark transition-all">
            Sign Up
          </router-link>
        </template>
        <template v-else>
...
export default {
  ...
  computed: {
    ...
    unreadCount() {
      return this.conversationList.filter(c => !c.isRead).length;
    },
    unreadNotifCount() {
